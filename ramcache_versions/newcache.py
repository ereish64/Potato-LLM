...
##############################################################################
def prefetch_worker(layer_idx, layers_dir: str, config, dtype: torch.dtype, patched_layers=None):
        ...
        while not stop_prefetch:
            ...
            try:
                ...
                with cache_condition:
                    layer_weights_cache[layer_idx] = decoder_layer.to(device)
                    scheduled_layers.discard(layer_idx)
                    scheduled_layers.add(layer_idx+NUM_PREFETCH_WORKERS)
                    prefetch_queue.put(layer_idx+NUM_PREFETCH_WORKERS)
...
if __name__ == "__main__":
    ...
    prefetch_threads = []
    for layer_idx in range(NUM_PREFETCH_WORKERS):
        thread = threading.Thread(
            target=prefetch_worker,
            args=(layer_idx, layers_dir, config, dtype, patched_layers),
            daemon=True
        )
        thread.start()
